...
_DEFAULT_CONTENT_MIX = (
    "40% educational/value, 30% personal/behind-the-scenes, 20% promotional, 10% engagement/questions"
)
...
class ContentCalendarSkill(BaseSkill):
    ...
    async def execute(self, params: dict, original_message: str) -> SkillResult:
        ...
        timing_lines = [
            f"  {p.title()}: {_PLATFORM_BEST_TIMES.get(p, 'research best time for your audience')}" for p in platforms
        ]

        # Available formats per platform
        format_lines = [
            f"  {p.title()}: {', '.join(_FORMAT_OPTIONS.get(p, ['Post', 'Video', 'Story']))}" for p in platforms
        ]
