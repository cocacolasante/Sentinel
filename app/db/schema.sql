...
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS output TEXT DEFAULT '';
...
-- ── Sentinel Mesh Agent tables ─────────────────────────────────────────────────

-- Registered mesh agents (one row per remote server)
CREATE TABLE IF NOT EXISTS mesh_agents (
    ...
    ip_address      TEXT,
    os_name         TEXT,
    sentinel_env    TEXT        NOT NULL DEFAULT 'staging',
    agent_version   TEXT,
    git_sha         TEXT,
    hmac_secret     TEXT        NOT NULL,   -- SHA-256 hex of the raw secret
    is_connected    BOOLEAN     NOT NULL DEFAULT FALSE,
    is_revoked      BOOLEAN     NOT NULL DEFAULT FALSE,
    last_seen       TIMESTAMPTZ,
    last_heartbeat  JSONB       DEFAULT '{}',
    registered_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_mesh_agents_env       ON mesh_agents (sentinel_env);
CREATE INDEX IF NOT EXISTS idx_mesh_agents_connected ON mesh_agents (is_connected);

-- Heartbeat telemetry stream (written every 30s per agent)
...
CREATE INDEX IF NOT EXISTS idx_mesh_heartbeats_agent ON mesh_heartbeats (agent_id, received_at DESC);

-- Code patch lifecycle (generated by Brain, applied by agent)
CREATE TABLE IF NOT EXISTS mesh_patches (
    id              BIGSERIAL   PRIMARY KEY,
    agent_id        UUID        NOT NULL REFERENCES mesh_agents(agent_id) ON DELETE CASCADE,
    patch_id        UUID        NOT NULL UNIQUE DEFAULT gen_random_uuid(),
    triggered_by    TEXT,       -- 'log_error' | 'manual' | 'sentry'
    diff_text       TEXT,
    files_changed   JSONB       DEFAULT '[]',
    status          TEXT        NOT NULL DEFAULT 'pending'
                                CHECK (status IN ('pending','approved','dispatched','applied','rolled_back','failed')),
...
CREATE INDEX IF NOT EXISTS idx_mesh_patches_agent  ON mesh_patches (agent_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mesh_patches_status ON mesh_patches (status, created_at DESC);

-- Audit log for all agent WebSocket messages (inbound + outbound)
CREATE TABLE IF NOT EXISTS mesh_audit_log (
    id              BIGSERIAL   PRIMARY KEY,
    agent_id        UUID,
    event_type      TEXT        NOT NULL,
